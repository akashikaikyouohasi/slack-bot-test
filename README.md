...
| `file_read` | Terraformリポジトリのファイル読み取り・検索 |
...
| [AWS Billing and Cost Management MCP Server](https://github.com/awslabs/mcp/tree/main/src/billing-cost-management-mcp-server) | AWSコスト分析・料金確認・コスト最適化推奨 |
...
```bash
# Processor Lambdaのパッケージング（Terraformリポジトリを含める場合）
TERRAFORM_REPO_URL=git@bitbucket.org:team/infra.git ./scripts/package.sh

# Terraformリポジトリなしでパッケージング
./scripts/package.sh
